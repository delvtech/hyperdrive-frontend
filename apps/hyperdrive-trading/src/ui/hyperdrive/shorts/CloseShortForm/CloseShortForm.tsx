...
export function CloseShortForm({
  ...
  const expiryDate = new Date(
    current.setMonth(
      current.getMonth() + convertMillisecondsToMonths(hyperdrive.termLengthMS),
    ),
  );
    ...
    <div className="flex flex-col gap-6">
        ...
        <div className="space-y-4 text-center text-base-content">
          <Stat
            ...
            value={
              baseAmountOut
                ? `${formatBalance(
                    formatUnits(baseAmountOut, hyperdrive.baseToken.decimals),
                    8,
      ...
      {account ? (
        <button
          ...
          onClick={(e) => {
            // send the tx to the user's wallet for signing
            closeShort?.();

import { AppConfig } from "src/appconfig/AppConfig";
export const sepoliaAppConfig: AppConfig = {
  chainId: 11155111,
  tags: [
    {
      id: "yieldSource",
      description: "A hyperdrive market's yield source",
    },
  ],
  tokens: [
    {
      address: "0x60CcC29Ee65d935D47bf2916568320815f96C3B2",
      decimals: 18,
      places: 4,
      name: "Liquid staked Ether 2.0",
      symbol: "stETH",
      iconUrl: "https://cryptologos.cc/logos/steth-steth-logo.png?v=029",
      tags: ["yieldSource", "steth"],
      extensions: {
        shortName: "Lido stETH",
        protocol: "lido",
        isSharesPeggedToBase: true,
      },
    },
    {
      address: "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",
      name: "Ether",
      symbol: "ETH",
      decimals: 18,
      places: 4,
      tags: [],
      extensions: {},
      iconUrl: "https://cryptologos.cc/logos/ethereum-eth-logo.png?v=029",
    },
    {
      address: "0xF4A3178BC28DaE9a842837a3421f8652CBCdf7bF",
      decimals: 18,
      places: 4,
      name: "RocketPool ETH",
      symbol: "RETH",
      iconUrl:
        "https://cryptologos.cc/logos/rocket-pool-eth-reth-logo.svg?v=031",
      tags: ["yieldSource", "reth"],
      extensions: {
        shortName: "Rocket Pool ETH",
        protocol: "rocketPool",
        isSharesPeggedToBase: false,
      },
    },
    {
      address: "0x3B57E3b2531a376Ee5B248943Bb6DE99E333BB5D",
      decimals: 18,
      places: 4,
      name: "Renzo ezETH",
      symbol: "ezETH",
      iconUrl:
        "data:image/svg+xml;base64,",
      tags: ["yieldSource", "ezeth"],
      extensions: {
        shortName: "Renzo ezETH",
        protocol: "renzo",
        isSharesPeggedToBase: false,
      },
    },
    {
      address: "0x80191B6a6A8E2026209fB5d1e4e9CC9A73029511",
      decimals: 18,
      places: 2,
      name: "Hyperdrive DAI",
      symbol: "hyperdriveDAI",
      iconUrl:
        "https://assets.coingecko.com/coins/images/29837/large/2022-official-morpho-token.png?1696528764",
      tags: ["yieldSource", "erc4626"],
      extensions: {
        shortName: "MetaMorpho",
        protocol: "morpho",
        isSharesPeggedToBase: false,
      },
    },
    {
      address: "0x8fb0C5a09438B36E42c6A7c7fD25B73c140ED3A3",
      decimals: 18,
      places: 2,
      name: "DAI",
      symbol: "DAI",
      iconUrl:
        "https://cryptologos.cc/logos/multi-collateral-dai-dai-logo.png?v=029",
      tags: [],
      extensions: {},
    },
    {
      address: "0xAb1895c74902F2dbd40e76CB616d80e4aA780713",
      decimals: 18,
      places: 2,
      name: "Savings DAI",
      symbol: "SDAI",
      iconUrl: "https://etherscan.io/token/images/Badgedai_32.svg",
      tags: ["yieldSource", "erc4626"],
      extensions: {
        shortName: "Maker DSR",
        protocol: "maker",
        isSharesPeggedToBase: false,
      },
    },
  ],
  hyperdrives: [
    {
      address: "0x0776F49B14718Fd735F74aAe8eAa9782730115c3",
      version: "v1.0.6",
      name: "14d DAI-Maker DSR",
      decimals: 18,
      baseToken: "0x8fb0C5a09438B36E42c6A7c7fD25B73c140ED3A3",
      sharesToken: "0xAb1895c74902F2dbd40e76CB616d80e4aA780713",
      depositOptions: {
        isBaseTokenDepositEnabled: true,
        isShareTokenDepositsEnabled: true,
      },
      withdrawOptions: {
        isBaseTokenWithdrawalEnabled: true,
        isShareTokenWithdrawalEnabled: true,
      },
      poolConfig: {
        baseToken: "0x8fb0C5a09438B36E42c6A7c7fD25B73c140ED3A3",
        vaultSharesToken: "0xAb1895c74902F2dbd40e76CB616d80e4aA780713",
        linkerFactory: "0x406346776319129C81B18ada0f6c7481AeD1BF48",
        linkerCodeHash:
          "0x9a69ff3b9a110deacb7d941d71222642f269891c0da9fcc15ec7c63fbd216b3f",
        initialVaultSharePrice: 1000000000000000000n,
        minimumShareReserves: 10000000000000000000n,
        minimumTransactionAmount: 1000000000000000n,
        circuitBreakerDelta: 600000000000000000n,
        positionDuration: 1209600n,
        checkpointDuration: 86400n,
        timeStretch: 3571858958603589n,
        governance: "0xc187a246Ee5A4Fe4395a8f6C0f9F2AA3A5a06e9b",
        feeCollector: "0xc187a246Ee5A4Fe4395a8f6C0f9F2AA3A5a06e9b",
        sweepCollector: "0xc187a246Ee5A4Fe4395a8f6C0f9F2AA3A5a06e9b",
        fees: {
          curve: 10000000000000000n,
          flat: 19230769230769n,
          governanceLP: 150000000000000000n,
          governanceZombie: 30000000000000000n,
        },
      },
    },
    {
      address: "0xC2dedf898EFd2a3c0467d37f08bb38dd212E2DAC",
      version: "v1.0.6",
      name: "30d DAI-Maker DSR",
      decimals: 18,
      baseToken: "0x8fb0C5a09438B36E42c6A7c7fD25B73c140ED3A3",
      sharesToken: "0xAb1895c74902F2dbd40e76CB616d80e4aA780713",
      depositOptions: {
        isBaseTokenDepositEnabled: true,
        isShareTokenDepositsEnabled: true,
      },
      withdrawOptions: {
        isBaseTokenWithdrawalEnabled: true,
        isShareTokenWithdrawalEnabled: true,
      },
      poolConfig: {
        baseToken: "0x8fb0C5a09438B36E42c6A7c7fD25B73c140ED3A3",
        vaultSharesToken: "0xAb1895c74902F2dbd40e76CB616d80e4aA780713",
        linkerFactory: "0x406346776319129C81B18ada0f6c7481AeD1BF48",
        linkerCodeHash:
          "0x9a69ff3b9a110deacb7d941d71222642f269891c0da9fcc15ec7c63fbd216b3f",
        initialVaultSharePrice: 1000007321156773211n,
        minimumShareReserves: 10000000000000000000n,
        minimumTransactionAmount: 1000000000000000n,
        circuitBreakerDelta: 600000000000000000n,
        positionDuration: 2592000n,
        checkpointDuration: 86400n,
        timeStretch: 7637309604522233n,
        governance: "0xc187a246Ee5A4Fe4395a8f6C0f9F2AA3A5a06e9b",
        feeCollector: "0xc187a246Ee5A4Fe4395a8f6C0f9F2AA3A5a06e9b",
        sweepCollector: "0xc187a246Ee5A4Fe4395a8f6C0f9F2AA3A5a06e9b",
        fees: {
          curve: 10000000000000000n,
          flat: 41666666666666n,
          governanceLP: 150000000000000000n,
          governanceZombie: 30000000000000000n,
        },
      },
    },
    {
      address: "0x74B556fbD2284c2aa7c673773572303E161E153B",
      version: "v1.0.6",
      name: "14d ETH-Lido stETH",
      decimals: 18,
      baseToken: "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",
      sharesToken: "0x60CcC29Ee65d935D47bf2916568320815f96C3B2",
      depositOptions: {
        isBaseTokenDepositEnabled: false,
        isShareTokenDepositsEnabled: true,
      },
      withdrawOptions: {
        isBaseTokenWithdrawalEnabled: false,
        isShareTokenWithdrawalEnabled: true,
      },
      poolConfig: {
        baseToken: "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",
        vaultSharesToken: "0x60CcC29Ee65d935D47bf2916568320815f96C3B2",
        linkerFactory: "0x406346776319129C81B18ada0f6c7481AeD1BF48",
        linkerCodeHash:
          "0x9a69ff3b9a110deacb7d941d71222642f269891c0da9fcc15ec7c63fbd216b3f",
        initialVaultSharePrice: 1000000772450532720n,
        minimumShareReserves: 1000000000000000n,
        minimumTransactionAmount: 1000000000000000n,
        circuitBreakerDelta: 600000000000000000n,
        positionDuration: 1209600n,
        checkpointDuration: 86400n,
        timeStretch: 1746050381163618n,
        governance: "0xc187a246Ee5A4Fe4395a8f6C0f9F2AA3A5a06e9b",
        feeCollector: "0xc187a246Ee5A4Fe4395a8f6C0f9F2AA3A5a06e9b",
        sweepCollector: "0xc187a246Ee5A4Fe4395a8f6C0f9F2AA3A5a06e9b",
        fees: {
          curve: 10000000000000000n,
          flat: 19230769230769n,
          governanceLP: 150000000000000000n,
          governanceZombie: 30000000000000000n,
        },
      },
    },
    {
      address: "0xe74F13a3E38f1a0bD867255B547d837cbCF2e8Aa",
      version: "v1.0.6",
      name: "30d ETH-Lido stETH",
      decimals: 18,
      baseToken: "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",
      sharesToken: "0x60CcC29Ee65d935D47bf2916568320815f96C3B2",
      depositOptions: {
        isBaseTokenDepositEnabled: false,
        isShareTokenDepositsEnabled: true,
      },
      withdrawOptions: {
        isBaseTokenWithdrawalEnabled: false,
        isShareTokenWithdrawalEnabled: true,
      },
      poolConfig: {
        baseToken: "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",
        vaultSharesToken: "0x60CcC29Ee65d935D47bf2916568320815f96C3B2",
        linkerFactory: "0x406346776319129C81B18ada0f6c7481AeD1BF48",
        linkerCodeHash:
          "0x9a69ff3b9a110deacb7d941d71222642f269891c0da9fcc15ec7c63fbd216b3f",
        initialVaultSharePrice: 1000000892313601400n,
        minimumShareReserves: 1000000000000000n,
        minimumTransactionAmount: 1000000000000000n,
        circuitBreakerDelta: 600000000000000000n,
        positionDuration: 2592000n,
        checkpointDuration: 86400n,
        timeStretch: 3737448720271234n,
        governance: "0xc187a246Ee5A4Fe4395a8f6C0f9F2AA3A5a06e9b",
        feeCollector: "0xc187a246Ee5A4Fe4395a8f6C0f9F2AA3A5a06e9b",
        sweepCollector: "0xc187a246Ee5A4Fe4395a8f6C0f9F2AA3A5a06e9b",
        fees: {
          curve: 10000000000000000n,
          flat: 41666666666666n,
          governanceLP: 150000000000000000n,
          governanceZombie: 30000000000000000n,
        },
      },
    },
    {
      address: "0xA975225D750dFC948171710739F1b3BBAd5d7451",
      version: "v1.0.6",
      name: "14d ETH-Rocket Pool ETH",
      decimals: 18,
      baseToken: "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",
      sharesToken: "0xF4A3178BC28DaE9a842837a3421f8652CBCdf7bF",
      depositOptions: {
        isBaseTokenDepositEnabled: false,
        isShareTokenDepositsEnabled: true,
      },
      withdrawOptions: {
        isBaseTokenWithdrawalEnabled: false,
        isShareTokenWithdrawalEnabled: true,
      },
      poolConfig: {
        baseToken: "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",
        vaultSharesToken: "0xF4A3178BC28DaE9a842837a3421f8652CBCdf7bF",
        linkerFactory: "0x406346776319129C81B18ada0f6c7481AeD1BF48",
        linkerCodeHash:
          "0x9a69ff3b9a110deacb7d941d71222642f269891c0da9fcc15ec7c63fbd216b3f",
        initialVaultSharePrice: 1000000785768645350n,
        minimumShareReserves: 1000000000000000n,
        minimumTransactionAmount: 1000000000000000n,
        circuitBreakerDelta: 600000000000000000n,
        positionDuration: 1209600n,
        checkpointDuration: 86400n,
        timeStretch: 1746050381163618n,
        governance: "0xc187a246Ee5A4Fe4395a8f6C0f9F2AA3A5a06e9b",
        feeCollector: "0xc187a246Ee5A4Fe4395a8f6C0f9F2AA3A5a06e9b",
        sweepCollector: "0xc187a246Ee5A4Fe4395a8f6C0f9F2AA3A5a06e9b",
        fees: {
          curve: 10000000000000000n,
          flat: 19230769230769n,
          governanceLP: 150000000000000000n,
          governanceZombie: 30000000000000000n,
        },
      },
    },
    {
      address: "0x5a28f5Dc994d231DBfdcF3Eb7F450f5bdF55d3A9",
      version: "v1.0.6",
      name: "30d ETH-Rocket Pool ETH",
      decimals: 18,
      baseToken: "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",
      sharesToken: "0xF4A3178BC28DaE9a842837a3421f8652CBCdf7bF",
      depositOptions: {
        isBaseTokenDepositEnabled: false,
        isShareTokenDepositsEnabled: true,
      },
      withdrawOptions: {
        isBaseTokenWithdrawalEnabled: false,
        isShareTokenWithdrawalEnabled: true,
      },
      poolConfig: {
        baseToken: "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",
        vaultSharesToken: "0xF4A3178BC28DaE9a842837a3421f8652CBCdf7bF",
        linkerFactory: "0x406346776319129C81B18ada0f6c7481AeD1BF48",
        linkerCodeHash:
          "0x9a69ff3b9a110deacb7d941d71222642f269891c0da9fcc15ec7c63fbd216b3f",
        initialVaultSharePrice: 1000000892313580475n,
        minimumShareReserves: 1000000000000000n,
        minimumTransactionAmount: 1000000000000000n,
        circuitBreakerDelta: 600000000000000000n,
        positionDuration: 2592000n,
        checkpointDuration: 86400n,
        timeStretch: 3737448720271234n,
        governance: "0xc187a246Ee5A4Fe4395a8f6C0f9F2AA3A5a06e9b",
        feeCollector: "0xc187a246Ee5A4Fe4395a8f6C0f9F2AA3A5a06e9b",
        sweepCollector: "0xc187a246Ee5A4Fe4395a8f6C0f9F2AA3A5a06e9b",
        fees: {
          curve: 10000000000000000n,
          flat: 41666666666666n,
          governanceLP: 150000000000000000n,
          governanceZombie: 30000000000000000n,
        },
      },
    },
    {
      address: "0xdeC715C6EAbad704A50deCB400bb18Ef4cde2240",
      version: "v1.0.6",
      name: "14d ETH-Renzo ezETH",
      decimals: 18,
      baseToken: "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",
      sharesToken: "0x3B57E3b2531a376Ee5B248943Bb6DE99E333BB5D",
      depositOptions: {
        isBaseTokenDepositEnabled: false,
        isShareTokenDepositsEnabled: true,
      },
      withdrawOptions: {
        isBaseTokenWithdrawalEnabled: false,
        isShareTokenWithdrawalEnabled: true,
      },
      poolConfig: {
        baseToken: "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",
        vaultSharesToken: "0x3B57E3b2531a376Ee5B248943Bb6DE99E333BB5D",
        linkerFactory: "0x406346776319129C81B18ada0f6c7481AeD1BF48",
        linkerCodeHash:
          "0x9a69ff3b9a110deacb7d941d71222642f269891c0da9fcc15ec7c63fbd216b3f",
        initialVaultSharePrice: 1000000799086757990n,
        minimumShareReserves: 1000000000000000n,
        minimumTransactionAmount: 1000000000000000n,
        circuitBreakerDelta: 600000000000000000n,
        positionDuration: 1209600n,
        checkpointDuration: 86400n,
        timeStretch: 1746050381163618n,
        governance: "0xc187a246Ee5A4Fe4395a8f6C0f9F2AA3A5a06e9b",
        feeCollector: "0xc187a246Ee5A4Fe4395a8f6C0f9F2AA3A5a06e9b",
        sweepCollector: "0xc187a246Ee5A4Fe4395a8f6C0f9F2AA3A5a06e9b",
        fees: {
          curve: 10000000000000000n,
          flat: 19230769230769n,
          governanceLP: 150000000000000000n,
          governanceZombie: 30000000000000000n,
        },
      },
    },
    {
      address: "0x496c57E03B63911ED37cd1ffc95d49b60AA22107",
      version: "v1.0.6",
      name: "30d ETH-Renzo ezETH",
      decimals: 18,
      baseToken: "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",
      sharesToken: "0x3B57E3b2531a376Ee5B248943Bb6DE99E333BB5D",
      depositOptions: {
        isBaseTokenDepositEnabled: false,
        isShareTokenDepositsEnabled: true,
      },
      withdrawOptions: {
        isBaseTokenWithdrawalEnabled: false,
        isShareTokenWithdrawalEnabled: true,
      },
      poolConfig: {
        baseToken: "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",
        vaultSharesToken: "0x3B57E3b2531a376Ee5B248943Bb6DE99E333BB5D",
        linkerFactory: "0x406346776319129C81B18ada0f6c7481AeD1BF48",
        linkerCodeHash:
          "0x9a69ff3b9a110deacb7d941d71222642f269891c0da9fcc15ec7c63fbd216b3f",
        initialVaultSharePrice: 1000000918949806795n,
        minimumShareReserves: 1000000000000000n,
        minimumTransactionAmount: 1000000000000000n,
        circuitBreakerDelta: 600000000000000000n,
        positionDuration: 2592000n,
        checkpointDuration: 86400n,
        timeStretch: 3737448720271234n,
        governance: "0xc187a246Ee5A4Fe4395a8f6C0f9F2AA3A5a06e9b",
        feeCollector: "0xc187a246Ee5A4Fe4395a8f6C0f9F2AA3A5a06e9b",
        sweepCollector: "0xc187a246Ee5A4Fe4395a8f6C0f9F2AA3A5a06e9b",
        fees: {
          curve: 10000000000000000n,
          flat: 41666666666666n,
          governanceLP: 150000000000000000n,
          governanceZombie: 30000000000000000n,
        },
      },
    },
    {
      address: "0xb4E605E079B4D9ed50B7202Ca0d008EE473A8de4",
      version: "v1.0.6",
      name: "30d DAI-MetaMorpho",
      decimals: 18,
      baseToken: "0x8fb0C5a09438B36E42c6A7c7fD25B73c140ED3A3",
      sharesToken: "0x80191B6a6A8E2026209fB5d1e4e9CC9A73029511",
      depositOptions: {
        isBaseTokenDepositEnabled: true,
        isShareTokenDepositsEnabled: false,
      },
      withdrawOptions: {
        isBaseTokenWithdrawalEnabled: true,
        isShareTokenWithdrawalEnabled: false,
      },
      poolConfig: {
        baseToken: "0x8fb0C5a09438B36E42c6A7c7fD25B73c140ED3A3",
        vaultSharesToken: "0x80191B6a6A8E2026209fB5d1e4e9CC9A73029511",
        linkerFactory: "0x406346776319129C81B18ada0f6c7481AeD1BF48",
        linkerCodeHash:
          "0x9a69ff3b9a110deacb7d941d71222642f269891c0da9fcc15ec7c63fbd216b3f",
        initialVaultSharePrice: 1000000000000000000n,
        minimumShareReserves: 10000000000000000000n,
        minimumTransactionAmount: 1000000000000000n,
        circuitBreakerDelta: 600000000000000000n,
        positionDuration: 2592000n,
        checkpointDuration: 86400n,
        timeStretch: 7637309604522233n,
        governance: "0xc187a246Ee5A4Fe4395a8f6C0f9F2AA3A5a06e9b",
        feeCollector: "0xc187a246Ee5A4Fe4395a8f6C0f9F2AA3A5a06e9b",
        sweepCollector: "0xc187a246Ee5A4Fe4395a8f6C0f9F2AA3A5a06e9b",
        fees: {
          curve: 10000000000000000n,
          flat: 41666666666666n,
          governanceLP: 150000000000000000n,
          governanceZombie: 30000000000000000n,
        },
      },
    },
    {
      address: "0x2F8702a0f20Bd6C152381D59a39DBe8cA87db9c2",
      version: "v1.0.6",
      name: "14d DAI-MetaMorpho",
      decimals: 18,
      baseToken: "0x8fb0C5a09438B36E42c6A7c7fD25B73c140ED3A3",
      sharesToken: "0x80191B6a6A8E2026209fB5d1e4e9CC9A73029511",
      depositOptions: {
        isBaseTokenDepositEnabled: true,
        isShareTokenDepositsEnabled: false,
      },
      withdrawOptions: {
        isBaseTokenWithdrawalEnabled: true,
        isShareTokenWithdrawalEnabled: false,
      },
      poolConfig: {
        baseToken: "0x8fb0C5a09438B36E42c6A7c7fD25B73c140ED3A3",
        vaultSharesToken: "0x80191B6a6A8E2026209fB5d1e4e9CC9A73029511",
        linkerFactory: "0x406346776319129C81B18ada0f6c7481AeD1BF48",
        linkerCodeHash:
          "0x9a69ff3b9a110deacb7d941d71222642f269891c0da9fcc15ec7c63fbd216b3f",
        initialVaultSharePrice: 1000000000000000000n,
        minimumShareReserves: 10000000000000000000n,
        minimumTransactionAmount: 1000000000000000n,
        circuitBreakerDelta: 600000000000000000n,
        positionDuration: 1209600n,
        checkpointDuration: 86400n,
        timeStretch: 3571858958603589n,
        governance: "0xc187a246Ee5A4Fe4395a8f6C0f9F2AA3A5a06e9b",
        feeCollector: "0xc187a246Ee5A4Fe4395a8f6C0f9F2AA3A5a06e9b",
        sweepCollector: "0xc187a246Ee5A4Fe4395a8f6C0f9F2AA3A5a06e9b",
        fees: {
          curve: 10000000000000000n,
          flat: 19230769230769n,
          governanceLP: 150000000000000000n,
          governanceZombie: 30000000000000000n,
        },
      },
    },
  ],
  protocols: {
    maker: {
      id: "maker",
      name: "Maker",
      iconUrl: "https://cryptologos.cc/logos/maker-mkr-logo.png?v=024",
    },
    lido: {
      id: "lido",
      name: "Lido",
      iconUrl: "https://cryptologos.cc/logos/lido-dao-ldo-logo.png?v=029",
    },
    morpho: {
      id: "morpho",
      name: "Morpho",
      iconUrl:
        "https://assets.coingecko.com/coins/images/29837/large/2022-official-morpho-token.png?1696528764",
    },
    rocketPool: {
      id: "rocketPool",
      name: "Rocket Pool",
      iconUrl:
        "https://assets.coingecko.com/coins/images/2090/large/rocket_pool_%28RPL%29.png?1696503058",
    },
    renzo: {
      id: "renzo",
      name: "Renzo",
      iconUrl:
        "data:image/svg+xml;base64,",
    },
  },
};
